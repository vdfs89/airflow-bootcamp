...
def postgres_to_snowflake_etl():
    ...
    def build_get_max_task(table_name: str):
        ...
        return get_max_primary_key(table_name)

    def build_load_task(table_name: str, max_id):
        ...
        def load_incremental_data(table_name: str, max_id: int):
            ...
            with SnowflakeHook(snowflake_conn_id="snowflake").get_conn() as sf_conn:
                with sf_conn.cursor() as sf_cursor:
                    insert_query = (
                        f"INSERT INTO {table_name} ({columns_list_str}) VALUES ({placeholders})"
                    )
                    for row in rows:
                        sf_cursor.execute(insert_query, row)
                sf_conn.commit()

        return load_incremental_data(table_name, max_id)

    for table_name in table_names:
        max_id = build_get_max_task(table_name)
        build_load_task(table_name, max_id)
